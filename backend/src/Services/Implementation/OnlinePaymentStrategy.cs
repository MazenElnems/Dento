...
/// <summary>
/// Handles online payments via the Paymob gateway.
/// Extracted from the original PaymentService.CreatePaymentIntent.
/// </summary>
public class OnlinePaymentStrategy : IPaymentStrategy
{
    ...
    public PaymentType SupportedPaymentType => PaymentType.Online;

    public OnlinePaymentStrategy(
        AppDbContext context,
        IOptions<PaymobSettings> paymob,
    ...
    {
        _logger.LogInformation("Creating online payment intent | AppointmentId: {AppointmentId} | PatientId: {PatientId}",
        ...
        {
            _logger.LogWarning("Unauthorized payment attempt | AppointmentId: {AppointmentId} | RequestingPatientId: {PatientId}",
                appointmentId, patientId);
        ...
        {
            _logger.LogWarning("Payment intent rejected — slot lock expired | AppointmentId: {AppointmentId}", appointmentId);
        ...
        {
            _logger.LogWarning(ex, "Duplicate idempotency key '{IdempotencyKey}' detected.", payment.IdempotencyKey);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogError("Failed to create payment intent. Status: {StatusCode}, Body: {Body}", response.StatusCode, errorBody);
            payment.Status        = PaymentStatus.Failed;
        ...
        _logger.LogInformation(
            "Payment intent created successfully | PaymentId: {PaymentId} | IntentionId: {IntentionId} | Amount: {Amount}",
            payment.Id, payment.IntentionId, payment.Amount);
